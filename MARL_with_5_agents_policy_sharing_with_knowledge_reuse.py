...
def test_RL(agent,episode_now):
    ...
    for episode in range(1000):
        ...
        for step in range (STEP):
            ...
            for i in range(0,AgentNUM+1):
                ...
                if i<AgentNUM:
                    action[i] = agent.get_greedy_action(state[i])
                else:
                    done, action[i] = sto_run(exp_map, x[i], y[i])
    ...
    if np.mean(step_list)<mean_best:
        ...
        agent.saver.save(agent.sess, 'model_SP_best_00')

def main():

    
    agent = DQN()
    agent.model_dir = 'model_SP'
    agent.optimizer_dir = 'optimizer_SP'
    agent.saver.restore(agent.sess, 'model_SP_best')
    #for i in range (0,AgentNUM-1):
    #    agent[i].saver.restore(agent[i].sess, 'model_'+str(i))
    ...
    for episode in range(EPISODE):
        ...
        for i in range (0,4):
            for j in range (0,i)
                state[i]= state[i]+[0]
        '''      
        agent.epsilon -= agent.epsilon_step
        ...
        for step in range (STEP):
            ...
            for i in range(0,AgentNUM+1):
                ...
                # 猎物最后行动且是随机的，前面的四个猎人有预测的成分在
                if i<AgentNUM:
                    action[i] = agent.get_action(state[i])
                else:
                    done, action[i] = sto_run(exp_map, x[i], y[i])
            ...
            for i in range(0,AgentNUM):
                ...
                agent.percieve( last_state[i], action[i], reward[i], state[i], done or(step == STEP - 1))
            if done:
                break
            #print(last_state[0],action, state[0], reward)
